...
def check_raises(label, fn, exc=Exception):
    ...
    try:
        ...
        FAIL += 1
        msg = f"  [FAIL] {label}  — wrong exception {type(e).__name__}: {e}"
        ERRORS.append(msg); print(msg)
...
# 1. CALORIMETRY
# ─────────────────────────────────────────────────────────────
section("1. CALORIMETRY  q = mcΔT")

# q = m*c*ΔT : water 200 g heated 10 °C  → q = 200*4.18*10 = 8360 J
check("q = 200 g water, ΔT=10 K  → 8360 J",    cal_q(200, 4.18, 10),   8360.0)
check("q = 50 g iron, ΔT=25 K   → 561.25 J",   cal_q(50, 0.449, 25),   561.25)
...
# c = q / (m*ΔT)
check("c = 8360 / (200*10) = 4.18 J/g·K",       cal_c(8360, 200, 10),   4.18)

# ΔT = q / (m*c)
check("ΔT = 8360 / (200*4.18) = 10",            cal_dT(8360, 200, 4.18), 10.0)

# ΔT from T_final - T_initial
dT_manual = 35.0 - 22.5
check("ΔT = T_final - T_initial = 12.5",        dT_manual, 12.5)
...
check("Roundtrip q→m", m_rt, 150.0, tol=1e-9)
check("Roundtrip q→c", c_rt, 2.44,  tol=1e-9)
check("Roundtrip q→ΔT", dT_rt, 8.0, tol=1e-9)

# Error cases
check_raises("c=0 → ValueError", lambda: cal_m(100, 0, 10), ValueError)
check_raises("ΔT=0 → ValueError for cal_m", lambda: cal_m(100, 4.18, 0), ValueError)
check_raises("m=0 → ValueError for cal_c", lambda: cal_c(100, 0, 10), ValueError)
check_raises("c=0 → ValueError for cal_dT", lambda: cal_dT(100, 50, 0), ValueError)
...
# celsius_to_kelvin
check("0°C = 273.15 K",   celsius_to_kelvin(0),    273.15)
check("100°C = 373.15 K", celsius_to_kelvin(100),  373.15)
check("-273.15°C = 0 K",  celsius_to_kelvin(-273.15), 0.0)
...
# Example: find ΔH for C + O2 → CO2
# Using: (1) C + 1/2 O2 → CO      ΔH1 = -110.5 kJ  (keep, ×+1)
#        (2) CO + 1/2 O2 → CO2    ΔH2 = -283.0 kJ  (keep, ×+1)
# Target: C + O2 → CO2  ΔH = -393.5 kJ
check("Hess: CO path → CO2 = -393.5 kJ",
      hess_law([-110.5, -283.0], [1, 1]), -393.5, tol=1e-9)

# Example: flip one reaction
# (1) 2H2 + O2 → 2H2O    ΔH = -572 kJ  (flip → 2H2O → 2H2 + O2, ×-1)
# (2) C + O2 → CO2        ΔH = -393.5 kJ  (keep, ×+1)
# ΔH_total = 572 + (-393.5) = +178.5 kJ
check("Hess: flip + keep = +178.5 kJ",
      hess_law([-572, -393.5], [-1, 1]), 178.5, tol=1e-9)

# Scale a reaction by 0.5
check("Hess: scale 0.5 × (-572) = -286 kJ",
      hess_law([-572], [0.5]), -286.0, tol=1e-9)

# Three-step Hess's law
# Standard formation of C3H8 (propane): known ΔH°f = -103.8 kJ
...
mults     = [3,       4,      -1]
# 3×(-393.5) + 4×(-285.8) + (-1)×(2220.0) = -1180.5 - 1143.2 - 2220.0 = ... closure check
...
check("H2+Cl2: ΔH ≈ -184 kJ",          dH2, -184.0, tol=1e-9)

# N2 + 3H2 → 2NH3 (Haber process)
# Broken: N#N (945) + 3 H-H (3×436=1308) = 2253 kJ
...
check("Haber: ΔH ≈ -93 kJ",            dH3,  -93.0, tol=1e-9)

# Zero bonds
dH_zero, sb_zero, sf_zero = bond_enthalpy_dH([], [])
check("Empty bonds: ΔH = 0",   dH_zero, 0.0)
check("Empty bonds: sb = 0",   sb_zero, 0.0)
check("Empty bonds: sf = 0",   sf_zero, 0.0)

# lookup_bond — canonical key
check("lookup C-H = 414",       lookup_bond("C-H"),  414)
...
# ΔH°f: CH4 = -74.8, O2 = 0, CO2 = -393.5, H2O(l) = -285.8 kJ/mol
...
check("CH4 combustion ΔH°rxn = -890.3 kJ", dH_rxn_ch4, -890.3, tol=0.01)

# Formation of water: H2 + 1/2 O2 → H2O(l)  ΔH°f(H2O) = -285.8 kJ
species_h2o = [
...
check("H2O formation ΔH°rxn = -285.8 kJ",
      standard_enthalpy_rxn(species_h2o), -285.8, tol=0.01)

# Decomposition (endothermic): 2 H2O → 2 H2 + O2  ΔH = +571.6 kJ
species_decomp = [
...
check("H2O decomposition ΔH°rxn = +571.6 kJ",
      standard_enthalpy_rxn(species_decomp), 571.6, tol=0.01)

# Coefficient scaling: 2 CO + O2 → 2 CO2
# ΔH°rxn = 2×(-393.5) - [2×(-110.5) + 1×0] = -787 - (-221) = -566 kJ
species_co = [
...
# ΔG = ΔH - T*ΔS
# Example: ΔH = -100 kJ, T = 300 K, ΔS = -200 J/K
# ΔG = -100 - 300*(-200/1000) = -100 + 60 = -40 kJ
check("ΔG = -100 - 300*(-0.2) = -40 kJ",
      gibbs_dG(-100, 300, -200), -40.0, tol=1e-9)

# ΔG = ΔH - TΔS: ΔH=-286, T=298, ΔS=-163.4 J/K
# ΔG = -286 - 298*(-163.4/1000) = -286 + 48.69 = -237.31 kJ  (approx. water formation)
dG_water = gibbs_dG(-285.8, 298, -163.4)
check("H2O formation ΔG ≈ -237.1 kJ", dG_water, -237.1, tol=0.5)

# Solve for ΔH
dH_back = gibbs_dH(-40, 300, -200)
check("dH from dG: ΔH = -100 kJ", dH_back, -100.0, tol=1e-9)

# Solve for ΔS
dS_back = gibbs_dS(-100, -40, 300)
check("dS from dG,dH,T: ΔS = -200 J/K", dS_back, -200.0, tol=1e-9)

# Solve for T (crossover, ΔG = 0)
# At equilibrium: T = ΔH / (ΔS/1000) = -100 / (-200/1000) = 500 K
T_cross = gibbs_T(-100, 0, -200)
check("Crossover T: -100 kJ / (-0.2 kJ/K) = 500 K", T_cross, 500.0, tol=1e-9)

# Celsius conversion
check("25°C = 298.15 K", celsius_to_kelvin(25), 298.15, tol=1e-9)

# Spontaneity labels
check("ΔG = -1  → spontaneous",     spontaneity(-1.0),    "Spontaneous (ΔG < 0)")
check("ΔG = +1  → non-spontaneous", spontaneity(+1.0),    "Non-spontaneous (ΔG > 0)")
check("ΔG = 0   → equilibrium",     spontaneity(0.0),     "At equilibrium (ΔG = 0)")
check("ΔG very small neg → equilib", spontaneity(-1e-10),  "At equilibrium (ΔG = 0)")
check("ΔG = -1e-5 → spontaneous",   spontaneity(-1e-5),   "Spontaneous (ΔG < 0)")

# Spontaneity analysis (qualitative)
check("dH<0, dS>0 → always spontaneous",
      spontaneity_analysis(-100, 100),
      "Always spontaneous at all temperatures (ΔH<0, ΔS>0)")
check("dH>0, dS<0 → never spontaneous",
      spontaneity_analysis(100, -100),
      "Never spontaneous at any temperature (ΔH>0, ΔS<0)")
check("dH<0, dS<0 → spontaneous low T",
      spontaneity_analysis(-100, -100),
      "Spontaneous at low T only (ΔH<0, ΔS<0 — enthalpy driven)")
check("dH>0, dS>0 → spontaneous high T",
      spontaneity_analysis(100, 100),
      "Spontaneous at high T only (ΔH>0, ΔS>0 — entropy driven)")
...
# Roundtrip: compute ΔG, then recover ΔH, ΔS, T
